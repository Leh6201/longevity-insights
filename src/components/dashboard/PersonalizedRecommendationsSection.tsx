...
import { Lightbulb, TrendingUp, CheckCircle2, ChevronRight, Sparkles, Heart, Apple, Footprints, Lock } from 'lucide-react';
...
const parseRecommendations = (rawRecommendations: string[]): Recommendation[] => {
  ...
  return rawRecommendations.slice(0, 5).map((rec, index) => {
    ...
    return {
      ...
      actions: sentences.slice(2).map(s => s.trim()).filter(s => s.length > 0).slice(0, 2) || ['Consulte um profissional de saúde para orientação personalizada'],
...
const PersonalizedRecommendationsSection: React.FC<PersonalizedRecommendationsSectionProps> = ({
    ...
    return <Card className="overflow-hidden min-w-0">
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between gap-2 min-w-0">
            <CardTitle className="flex items-start gap-3 text-lg min-w-0 flex-1">
              <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-amber-500 to-yellow-400 flex items-center justify-center shadow-md shrink-0 mt-0.5">
                <Sparkles className="w-5 h-5 text-white" />
              </div>
              <span className="min-w-0 flex-1 leading-tight">
                <span className="sm:hidden">Recomendações<br />Personalizadas</span>
                <span className="hidden sm:inline">Recomendações Personalizadas</span>
              </span>
            </CardTitle>
        ...
        <CardContent className="relative">
          ...
          <div className="blur-sm pointer-events-none select-none space-y-4">
            {[1, 2, 3].map((_, idx) => <div key={idx} className="p-4 rounded-xl bg-muted/50 space-y-2">
          ...
          {/* Lock Overlay */}
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-background/60 backdrop-blur-[2px] cursor-pointer" onClick={() => navigate('/premium')}>
  ...
  return <Card className="overflow-hidden min-w-0">
      ...
      <CardContent className="space-y-4">
        {structuredRecommendations.length > 0 ? structuredRecommendations.map((rec, idx) => {
          ...
          delay: idx * 0.1
                ...
                <div className="space-y-1.5">
                  ...
                  <ul className="space-y-1 pl-5">
                    {rec.actions.length > 0 ? rec.actions.map((action, actionIdx) => <li key={actionIdx} className="text-sm text-foreground/80 flex items-start gap-2">
                          <ChevronRight className="w-3.5 h-3.5 mt-0.5 text-primary shrink-0" />
                          <span>{action}</span>
                        </li>) : <li className="text-sm text-foreground/80 flex items-start gap-2">
                        <ChevronRight className="w-3.5 h-3.5 mt-0.5 text-primary shrink-0" />
